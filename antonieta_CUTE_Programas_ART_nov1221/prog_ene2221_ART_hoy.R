...
library(devEMF)
source('CreaFolder.R')
source('SolucionXPU_Runge.R')
source('SolucionPPU_Runge.R')
source('gx.R')
source('gw.R')
source('Calu1u2.R')
...
for (jj in 1:40){
	...
	R <- SolucionXPU_Runge(del, pp, wx1=dwx1, wx2=dwx2)
	
	if (jj == kk){
		fig_nametX1 = paste(fig, prefijo, '_', jj,'_tX1_',kk, '.emf', sep='')
		emf(file = file.path(pat, prefijo,fig_nametX1), emfPlus = FALSE)
		par(mai=c(1,1,.5,.5))
	...
	pp <- SolucionPPU_Runge(del, R$xx, R$uu, wp=dwp)	
