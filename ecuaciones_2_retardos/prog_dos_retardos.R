...
procesa_izquierda2 <- function(tao1=0,tao2=0, niter=0, valfin=0){
  print('procesa izquierda2')
  del = .001
  n = tao2 / del+1  
  m = tao1 / del+1
  t = seq(tao1, 0, length.out = m)  
  yf = rep(valfin, n)
  ...
  for (i in 1:niter){
    ...
    y2 = yy[ni:(ni+m)]
    k = length(yy)
    yi = Res_Equ_Dif_derecha_tao2(yy[1] , del, y1, y2)
    yy = c(yy, yi)
    mi = mi+m
    ni = ni+m
  }
