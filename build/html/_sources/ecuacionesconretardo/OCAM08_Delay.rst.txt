...
Assume that r,s  0,(r,s) ̸ =(0,0) and

In particular, this assumption holds for any couple of rational numbers (r,s), where at least one number is 
nonzero.

En particular, esta suposición se cumple para cualquier par de números racionales (r,s), donde al menos uno de 
...
Obtendremos las condiciones necesarias de optimalidad para el problema de control retardado (ROCP) transformando 
(aumentando) dicho problema en un problema de control sin retardo de mayor dimensión. Para estudiar con mayor 
profundidad el problema aumentado, necesitamos el principio de mínimo de Pontryagin para problemas de control sin 
retardo con restricciones mixtas de control y estado, que se revisará en la siguiente sección.
...
Las siguientes condiciones necesarias de primer orden se pueden encontrar en Göllmann [ 14 ]; Kern [ 15 ] ha dado 
una demostración precisa bajo la suposición 4.1.
...
Teorema 4.2 (principio mínimo para el problema de control óptimo retardado (ROCP)) Sea (ˆu, ˆx) localmente óptimo 
para (ROCP) con retardos que satisfacen la suposición 4.1. Entonces existen una función coestado (adjunta) ˆ ∈W 
1,∞ ( [ a,b ] ,R n ), una función multiplicadora ˆ ∈ L ∞ ( [ a,b ] ,R p ) y un multiplicador ˆ ∈R q , tales 
que se cumplen las siguientes condiciones para casi todo t ∈ [ a,b ] :
...
Demostración La demostración utiliza una técnica de transformación sugerida por Guinn [ 6 ] para derivar 
condiciones necesarias de primer orden para problemas de control óptimo sin restricciones con retardos de estado 
puros. En vista del supuesto de racionalidad (17), existen enteros k,l ∈N con
...
Sin pérdida de generalidad, podemos asumir el primer caso. Entonces, los retrasos r,s son múltiplos enteros de la 
longitud del intervalo h :=s/l:
...
El intervalo de tiempo [ a,a + h ] se utilizará a continuación como intervalo de tiempo base para el problema de 
control aumentado. Sin pérdida de generalidad, podemos suponer además que la longitud del intervalo b−a 
representa un múltiplo entero de h, es decir, tenemos b−a = Nh con N ∈N + .
...
La continuidad del estado x(t) en [ a,b ] implica las siguientes condiciones de contorno para el estado aumentado 
(t):
...
En términos de las nuevas variables de estado y control y , el problema de control retardado (ROCP) es 
equivalente al siguiente problema de control óptimo sin retardo en el intervalo de tiempo [ a,a + h ] :

The ﬁxed starting proﬁles (3) and (4) are included in this notation by considering the variables • −k ,...,  
−1 and −l ,..., −l deﬁned by

Los perfiles iniciales fijos (3) y (4) se incluyen en esta notación al considerar las variables • −k ,…, −1 y 
−l ,…, −l definidas por
...
Sin embargo, tenga en cuenta que −k ,…, −1 y −l ,…, −1 no representan variables de optimización. 
Introduciendo variables adjuntas y multiplicadores para el problema aumentado por (26)–(29) por

the Hamiltonian functions (8) and (9) for the nondelayed augmented control problem are given by

Las funciones hamiltonianas (8) y (9) para el problema de control aumentado sin retardo vienen dadas por
...
Cada par óptimo local (ˆu(·), ˆx(·)) para (ROCP) define un par ( (·), (·)) que minimiza el problema aumentado 
(26)–(29). El principio mínimo de Pontryagin para problemas sin retardo (Teorema 3.1) asegura la existencia de 
una función adjunta (coestado) ˆ ∈W 1,∞ ( [ a,a + h ] ,R N·n ), una función multiplicadora M ˆ ∈ L ∞ ( [ a,a 
+ h ] ,R N·p ) y un vector ∈R (N−1)n + q , ˆ =(ˆ ∗ ,…, ˆ ∗ , ˆ ∗ )∗ 0 N−2 N−1 donde ˆ 0 ,… ˆ N−2 ∈R n y 
ˆ N−1 ∈R q , tales que las siguientes condiciones se cumplen para casi todo
...
Ahora podemos definir la función adjunta ˆ ∈W 1,∞ ( [ a,b ] ,R n ) y la función multiplicadora ˆ ∈ L ∞ ( [ 
a,b ] ,R p ) para el problema de control retardado (ROCP) de la siguiente manera. Para t ∈ [ a,b ] , existe 0 j 
N −1 con a + jh t a + ( j + 1)h. Sustituimos
...
Para verificar la condición mínima para el hamiltoniano H, consideramos t ∈ [ a,b ] y el índice correspondiente 
j ∈ { 0,…, N −1 } con a + jh t a + ( j + 1)h. Sustituyendo t ′ :=t − jh ∈ [ a,a + h ] , la condición mínima 
(36) da
...
para todo ∈R Nm admisible que satisfaga (29). La condición de mínimo local (37) produce K (t ′ )=0. Ahora 
definimos una política de control admisible (·)=( ∗ ,…, ∗ ) ∗ ∈R Nm mediante
...
donde el vector de control u ∈R m es admisible para (ROCP), es decir, C(t, ˆx(t),u) 0. Evaluando la desigualdad 
(40) para este vector y eliminando expresiones iguales en ambos lados, obtenemos para los términos restantes 
asociados con j y j + l:
...
Redefiniendo la función adjunta y multiplicadora en (39) con t ′ =t − jh, obtenemos la condición de mínimo 
deseada (21) para el hamiltoniano H, y respectivamente, la condición de mínimo local (22) para el hamiltoniano 
aumentado. La condición (38) implica inmediatamente la condición de multiplicador y complementariedad (23) en 
vista de (39).
...
Nota: Soliman y Ray [5] han analizado los controles de tipo bang-bang y singulares que aparecen en los problemas 
de control, donde el control u ∈R m se divide en controles u 1 ∈R m 1 y u 2 ∈R m 2, con el control u 1 
apareciendo linealmente en el sistema. La restricción de estado de control (6) se reduce entonces a límites para 
u 1:
...
La condición mínima (21) muestra que el control u 1 (t) está determinado por el signo de las componentes de la 
función del vector de conmutación.
...
El problema de control del reactor CSTR en la Sección 6 proporciona un ejemplo con dicha partición del vector de 
control. Soliman y Ray [5] estudian los fenómenos de unión para arcos de control de tipo bang-bang y singulares. 
No solo proporcionan condiciones bajo las cuales los resultados de unión para sistemas de control sin retardo se 
extienden a sistemas con retardo, sino que también ofrecen ejemplos de sistemas con retardo que presentan 
características inusuales. Kern [15] ha desarrollado algunos ejemplos que ilustran estas características 
inusuales. Se necesita más trabajo para desarrollar completamente la teoría.
